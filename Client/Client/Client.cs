...
namespace Client
{
    class Client
    {
        ...
        {// обработка ощибок
            try
            {
                // Отправка: Целочисленного значения (из диапазона 10-20).
                while (Work)
                {
                    Socket.Send(BitConverter.GetBytes(Convert.ToDouble(new Random().Next(10, 20))));
                    Console.WriteLine(DateTime.Now + " Request has been sent.");
                    Thread.Sleep(1600);
                }
            }
            catch (Exception e)
        ...
        {//Обработка ощибок
            try
            {
                while (Work)
                {
                    ...
                    Console.WriteLine(DateTime.Now + " The resulting number: " + BitConverter.ToInt32(answer, 0));
                }
            }
            catch (Exception e)
            {
